...
public class RedisCacheImpl implements RedisCache{
    ...
    public <T> T get(RedisKeyBuild redisKeyBuild, Class<T> clazz) {
        ...
        String value = redisTemplate.opsForValue().get(redisKeyBuild.getRelKey());
    ...
    public <T> T get(RedisKeyBuild redisKeyBuild, Class<T> clazz, Supplier<T> supplier, long ttl, TimeUnit timeUnit) {
        CacheUtil.checkNotBlank(redisKeyBuild);
        T value = get(redisKeyBuild, clazz);
        if (CacheUtil.isEmpty(value)) {
            value = supplier.get();
            if (CacheUtil.isEmpty(value)) {
                return null;
            }
            set(redisKeyBuild, value, ttl, timeUnit);
        }
        return value;
    }
    ...
    public List<String> getKeys(List<RedisKeyBuild> keyList) {
        ...
        List<String> keys = CacheUtil.getBatchKey(keyList);
    ...
    public Set<String> keys(String pattern) {
        CacheUtil.checkNotBlank(pattern);
    ...
    public void rename(RedisKeyBuild oldKey, RedisKeyBuild newKey) {
        ...
        String oldKeyStr = oldKey.getRelKey();
        String newKeyStr = newKey.getRelKey();
        redisTemplate.rename(oldKeyStr, newKeyStr);
    }
    ...
    public Boolean renameIfAbsent(RedisKeyBuild oldKey, RedisKeyBuild newKey) {
        ...
        String oldKeyStr = oldKey.getRelKey();
        String newKeyStr = newKey.getRelKey();
        return redisTemplate.renameIfAbsent(oldKeyStr, newKeyStr);
    }
    ...
    public <T> List<T> getAllForList(RedisKeyBuild redisKeyBuild, Class<T> clazz) {
        return rangeForList(redisKeyBuild, 0, -1, clazz);
    }
